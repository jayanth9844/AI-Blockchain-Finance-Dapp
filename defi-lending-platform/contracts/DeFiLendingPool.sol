...
contract DeFiLendingPool {
 ...
 // Calculate the total amount earned by a lender (deposit + interest)
...
// Calculate the total loan repayment (loan + interest) for a borrower
function calculateTotalRepayment(address borrower) public view returns (uint256) {
    ...
    // Calculate the total repayment (loan principal + interest)
    uint256 totalRepayment = loan.amount + interest;
    
    // Ensure the borrower sends at least the required total repayment
    require(msg.value >= totalRepayment, "Insufficient repayment amount");
    
    // If the borrower sends more than the required amount, refund the excess
    if (msg.value > totalRepayment) {
        payable(msg.sender).transfer(msg.value - totalRepayment);
    }

    // Update the liquidity and loan records
    totalBorrowed -= loan.amount;
    totalLiquidity += totalRepayment;

    // Delete the loan record after repayment
    delete loans[msg.sender];
    
    // Emit an event to log the repayment
    emit LoanRepaid(msg.sender, loan.amount);
}
